...
export default function MenuItems({ currentItems, items, searchedDish }) {
    ...
    useEffect(() => {
        if (searchedDish !== '') {
            setUpdatedDishes(items);
        }
        else {
            setUpdatedDishes(currentItems);
        }
    ...
    return (
        <>
            {updatedDishes && updatedDishes.filter(dish => dish.dish_name.toLowerCase().includes(searchedDish.toLowerCase()))
